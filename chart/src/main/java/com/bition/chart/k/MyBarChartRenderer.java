...
import com.bition.chart.k.bean.KLineBean;
    ...
    protected void drawDataSet(Canvas c, IBarDataSet dataSet, int index) {
        ...
        for (int j = 0; j < buffer.size(); j += 4) {
            ...
            if (!isSingleColor) {
                ...
                int i = j / 4;
                KLineBean kLineBean = (KLineBean) dataSet.getEntryForIndex(i).getData();
                if (kLineBean == null) {
                    mRenderPaint.setColor(dataSet.getColor(i));
                } else {
                    if (kLineBean.close < kLineBean.open) {
...
//
//        BarData barData = mChart.getBarData();
//        Log.i("drawHighlighted", indices.length + "");
//
//        for (Highlight high : indices) {
//
//            IBarDataSet set = barData.getDataSetByIndex(high.getDataSetIndex());
